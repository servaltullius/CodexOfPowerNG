...
- 기록은 `공격 / 방어 / 유틸` 빌드 점수로 누적됩니다.
- 누적 점수로 옵션을 해금하고, 활성 슬롯에 배치해 현재 빌드를 조립합니다.

플레이 체감 포인트:
- 아이템을 "쌓아두기만" 하지 않고 실제로 소모해 진행 목표를 만듭니다.
- 무엇을 기록했는지가 빌드 방향으로 이어져 수집 동기와 성장 선택이 연결됩니다.
...
- `F4`: 메뉴 열기/닫기
- Quick Register:
  - `공격 / 방어 / 유틸` 섹션에서 아이템을 선택 등록합니다.
  - 체크 후 `Register Selected`로 여러 행을 한 번에 등록할 수 있습니다.
- Build:
  - 기록 점수로 해금한 옵션을 보고, 슬롯에 활성화해 현재 빌드를 구성합니다.
- Settings:
  - 언어: `auto / en / ko`
...
- Undo 성공 메시지에 `[경고: 보상 롤백이 적용되지 않음]`이 붙음: 아이템 복구는 성공했지만 해당 액션의 진행도 변화가 이미 정리된 상태라 추가 롤백할 값이 없었다는 의미입니다.
...
- Records accumulate into `Attack / Defense / Utility` build score.
- Build score unlocks options, and active slots determine which effects shape the current build.

Why this feels different:
- It creates a meaningful item sink and collection objective.
- What you record directly opens different build directions instead of feeding a random reward roll.
...
- `F4`: Toggle menu
- Quick Register:
  - Register items from grouped `Attack / Defense / Utility` sections.
  - Use `Register Selected` to batch-register checked rows safely.
- Build:
  - Review unlocked options and slot the ones you want active in the current build.
- Settings:
  - Language: `auto / en / ko`
...
- Undo success message includes `[warning: reward rollback not applied]`: item restore succeeded, but there was no remaining progression delta to roll back for that action.
...
- `PrismaUI/views/codexofpowerng/ui_rendering.js`: status/list/build/settings rendering
