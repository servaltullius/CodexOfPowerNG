...
test("build panel renders catalog-first layout with discipline/theme navigation and detail-first rail", () => {
  ...
  assert.match(renderedHtml, /buildSlotMatrixScroller/);
  assert.match(renderedHtml, /buildSlotMatrixScroller[^>]*data-wheel-surface="build-slots"/);
  ...
  assert.match(renderedHtml, /buildSlotMatrixCard[\s\S]*buildSlotMatrixHeader[\s\S]*buildSlotMatrixName[\s\S]*buildSlotMatrixActions/);
  ...
  assert.match(renderedHtml, /비어 있음/);
  assert.doesNotMatch(renderedHtml, /옵션 대기 중/);
...
test("build view source includes a fixed catalog-first layout contract", () => {
  ...
  assert.match(html, /\.buildDetailRail\s*\{[\s\S]*grid-template-rows:\s*minmax\(0,\s*1fr\)\s*auto/);
  assert.match(html, /\.buildSlotMatrix\s*\{/);
  assert.match(html, /\.buildSlotSummaryPanel\s*\{[\s\S]*display:\s*flex/);
  assert.match(html, /\.buildSlotSummaryPanel\s*\{[\s\S]*flex-direction:\s*column/);
  assert.match(html, /\.buildSlotSummaryPanel\s*\{[\s\S]*overflow:\s*visible/);
  assert.match(html, /\.buildSlotMatrixScroller\s*\{[\s\S]*overflow:\s*auto/);
  assert.match(html, /\.buildSlotMatrixScroller\s*\{[\s\S]*overscroll-behavior:\s*contain/);
  assert.match(html, /\.buildSlotMatrixScroller\s*\{[\s\S]*max-height:\s*min\(calc\(176px \* var\(--uiScale\)\),\s*calc\(28vh\)\)/);
  ...
  assert.match(html, /\.buildSlotMatrix\s*\{[\s\S]*gap:\s*calc\(4px \* var\(--uiScale\)\)/);
  assert.match(html, /\.buildSlotMatrixCard\s*\{[\s\S]*grid-template-columns:\s*auto minmax\(0,\s*1fr\) auto/);
  assert.match(html, /\.buildSlotMatrixCard\s*\{[\s\S]*align-items:\s*center/);
  assert.match(html, /\.buildSlotMatrixCard\s*\{[\s\S]*padding:\s*calc\(6px \* var\(--uiScale\)\)\s*calc\(8px \* var\(--uiScale\)\)/);
  assert.match(html, /\.buildSlotMatrixName\s*\{[\s\S]*min-width:\s*0/);
  assert.match(html, /\.buildSlotMatrixName\s*\{[\s\S]*font-size:\s*calc\(11px \* var\(--uiScale\)\)/);
  assert.match(html, /\.buildSlotMatrixName\s*\{[\s\S]*white-space:\s*nowrap/);
  assert.match(html, /\.buildSlotMatrixName\s*\{[\s\S]*text-overflow:\s*ellipsis/);
  assert.match(html, /\.buildSlotMatrixActions\s*\{[\s\S]*flex-wrap:\s*nowrap/);
  assert.match(html, /\.buildSlotMatrixActions\s*\{[\s\S]*justify-content:\s*flex-end/);
  assert.match(html, /\.buildSlotMatrixActions\s+\.buildHint\s*\{[\s\S]*white-space:\s*nowrap/);
  assert.match(html, /\.buildSlotMatrixActions\s+\.buildHint\s*\{[\s\S]*text-overflow:\s*ellipsis/);
  assert.match(html, /\.buildSlotMatrixActions\s+\.buildActionButton\s*\{[\s\S]*min-height:\s*calc\(28px \* var\(--uiScale\)\)/);
  ...
  assert.match(html, /@media \(max-width: 980px\)\s*\{[\s\S]*\.buildSlotMatrixScroller\s*\{[\s\S]*max-height:\s*none/);
  assert.match(html, /@media \(max-width: 980px\)\s*\{[\s\S]*\.buildSlotMatrixScroller\s*\{[\s\S]*overflow:\s*visible/);
