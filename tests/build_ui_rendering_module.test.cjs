...
test("build view source includes a fixed catalog-first layout contract", () => {
  ...
  assert.match(html, /\.buildSlotMatrix\s*\{[\s\S]*gap:\s*calc\(5px \* var\(--uiScale\)\)/);
  assert.match(html, /\.buildSlotMatrixCard\s*\{[\s\S]*padding:\s*calc\(7px \* var\(--uiScale\)\)/);
  assert.match(html, /\.buildSlotMatrixName\s*\{[\s\S]*font-size:\s*calc\(11\.5px \* var\(--uiScale\)\)/);
  assert.match(html, /\.buildSlotMatrixActions\s+\.buildActionButton\s*\{[\s\S]*min-height:\s*calc\(30px \* var\(--uiScale\)\)/);
